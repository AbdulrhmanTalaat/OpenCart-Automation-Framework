...
```
TypeError: checkOut.clickOnContinueButtonvisible is not a function
```
...
> 74  |     const test = await checkOut.clickOnContinueButtonvisible();
      |                                 ^ TypeError: checkOut.clickOnContinueButtonvisible is not a function
  75  |     console.log(`Button is visible : ${test}`)
  76  |     //await checkOut.clickOnContinueButtonCheckoutOptions();
  77  | /*
  78  |     // Step 2: Billing Details 
  79  |     const Billing_Details = await checkOut.verifyTitleBillingDetailsIsVisible();
  80  |     expect(Billing_Details).toBeTruthy();
  81  |     console.log(`Are Step 2: Billing Details Is Appear in Correct : ${Billing_Details}`);
  82  | 
  83  |      await checkOut.fillYourPersonalDetails(RandomDataUtil.getlastName(),
  84  |                                            RandomDataUtil.getlastName() , RandomDataUtil.getEmail() , RandomDataUtil.getPhoneNumber());
  85  |     await checkOut.fillYourAddress(RandomDataUtil.getRandomAddress() , RandomDataUtil.getRandomCity() ,RandomDataUtil.getRandomPin());
  86  |     await checkOut.setCountry(config.country , config.state);
  87  |     await checkOut.clickOnbtnContinueBillingPersonalDetails();
  88  |   
  ...
  103 |            await checkOut.clickOnContinueButtonConfirmationOrder(); */
  104 | 
  105 | 
  106 | });
  107 | 
  108 | 
  109 | 
  110 | test.afterEach("Closs App", async ({ page }) => {
  111 |     await page.waitForTimeout(10000);
